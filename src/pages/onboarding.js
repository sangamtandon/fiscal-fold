  ...
  return `
    <div class="onboarding__bucket-wrap" data-id="${escapeHtml(bucket.id)}" data-macro="${macroType}">
      <div class="onboarding__bucket-item">
        ...
        <button class="onboarding__bucket-recurring ${bucket.isRecurring ? 'is-active' : ''}" data-id="${bucket.id}" data-macro="${macroType}" title="Fixed monthly bill — will be set up as a Commitment" aria-label="Fixed monthly bill — will be set up as a Commitment">
          🔄
        </button>
        <button class="onboarding__bucket-pin ${bucket.isPinned ? 'is-active' : ''}" data-id="${bucket.id}" data-macro="${macroType}" title="Pin as Quick Bucket" aria-label="Pin as Quick Bucket">
          📌
        </button>
        <button class="onboarding__bucket-remove" data-id="${bucket.id}" data-macro="${macroType}" title="Remove" aria-label="Remove">
          ×
        </button>
      </div>
      ${bucket.isRecurring ? `
