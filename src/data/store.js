...
export function getBuckets(macroType) {
  ...
  // ⚡ Bolt: Single-pass for loop instead of chained .filter() calls
  const buckets = [];
...
export function getMacroReserved(macroType) {
  // ⚡ Bolt: Single-pass for loop instead of chained .filter().reduce() calls
  let sum = 0;
...
export function getSafeToSpend() {
  ...
  // ⚡ Bolt: Single-pass for loop instead of .reduce()
  let remaining = 0;
...
export function getMacroSummary(macroType) {
  ...
  // ⚡ Bolt: Single-pass for loop instead of chained .reduce() calls
  // Reduces garbage collection overhead during frequent re-renders.
  let allocated = 0;
  let spent = 0;
  let swept = 0;
