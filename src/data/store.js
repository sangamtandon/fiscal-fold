...
export function getBuckets(macroType) {
  ...
  const buckets = [];
  // ⚡ Bolt Optimization: Single-pass filter to reduce GC overhead during frequent re-renders
...
export function getMacroReserved(macroType) {
  let reserved = 0;
  // ⚡ Bolt Optimization: Single-pass loop to reduce GC overhead during frequent re-renders
  for (let i = 0; i < _state.commitments.length; i++) {
    const c = _state.commitments[i];
