...
export function getBuckets(macroType) {
  ...
  // ⚡ Bolt: Single-pass loop replaces multiple .filter() calls to minimize intermediate array allocations
  const buckets = [];
...
export function getQuickBuckets() {
  ...
  // ⚡ Bolt: Single-pass loop replaces .filter() to prevent unnecessary array allocations
  const pinned = [];
  for (let i = 0; i < _state.buckets.length; i++) {
    const b = _state.buckets[i];
...
export function getTransactions({ limit, bucketId } = {}) {
  ...
  // ⚡ Bolt: Single-pass loop replaces multiple .filter() calls to eliminate intermediate arrays
  const txns = [];
...
export function getMacroReserved(macroType) {
  // ⚡ Bolt: Replaced .filter().reduce() chain with single-pass loop to eliminate intermediate array allocations
  let sum = 0;
...
export function getSafeToSpend() {
  ...
  // ⚡ Bolt: Single-pass loop replaces .reduce() to prevent callback overhead during rapid re-renders
  let remaining = 0;
...
export function getMacroSummary(macroType) {
  ...
  // ⚡ Bolt: Single-pass loop replaces multiple .reduce() calls to eliminate intermediate arrays and GC overhead
  let allocated = 0;
  let spent = 0;
  let swept = 0;
