...
export function getBuckets(macroType) {
  ...
  const buckets = [];
  // ⚡ Bolt: Single-pass for loop avoids intermediate array allocations from chained .filter()
...
export function getBucketById(id) {
  // ⚡ Bolt: single-pass lookup avoids filter array allocation
  for (let i = 0; i < _state.buckets.length; i++) {
    if (_state.buckets[i].id === id) return _state.buckets[i];
  }
  return undefined;
}
...
export function getQuickBuckets() {
  const cycleId = _state.currentCycleId;
  const buckets = [];
  // ⚡ Bolt: Single-pass for loop avoids chained .filter() overhead
  for (let i = 0; i < _state.buckets.length; i++) {
    const b = _state.buckets[i];
...
export function getTransactions({ limit, bucketId } = {}) {
  const cycleId = _state.currentCycleId;
  const txns = [];
  // ⚡ Bolt: Single-pass loop avoids .filter() chained allocations
...
export function getCommitments() {
  const active = [];
  // ⚡ Bolt: single pass loop avoids Array prototype overhead on frequent re-renders
  for (let i = 0; i < _state.commitments.length; i++) {
    if (_state.commitments[i].isActive) active.push(_state.commitments[i]);
  }
  return active;
}
...
export function getMacroReserved(macroType) {
  let sum = 0;
  // ⚡ Bolt: Single-pass for loop avoids chained .filter().reduce() GC garbage overhead
  for (let i = 0; i < _state.commitments.length; i++) {
...
export function getSafeToSpend() {
  ...
  let remaining = 0;
  // ⚡ Bolt: Single-pass for loop avoids .reduce() array GC overhead
...
export function getMacroSummary(macroType) {
  ...
  // ⚡ Bolt: Single-pass for loop avoids chained .reduce() GC garbage overhead
...
export function getCommitmentsTotal(macroType) {
  let sum = 0;
  // ⚡ Bolt: Single-pass for loop avoids chained .filter().reduce() GC overhead
  for (let i = 0; i < _state.commitments.length; i++) {
    const c = _state.commitments[i];
...
export function getSweeps(cycleId) {
  ...
  const sweeps = [];
  // ⚡ Bolt: Single-pass for loop avoids .filter() array GC overhead
