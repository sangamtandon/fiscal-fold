...
export function getMacroReserved(macroType) {
  // Optimization: Use a single-pass loop instead of `.filter().reduce()`
  // to avoid allocating intermediate arrays, reducing GC overhead.
  let sum = 0;
...
export function getSafeToSpend() {
  ...
  // Optimization: Replaced `.reduce()` with a single-pass `for` loop
  // to eliminate garbage collection overhead caused by function allocations in hot code paths.
  let remaining = 0;
...
export function getMacroSummary(macroType) {
  ...
  // Optimization: Combined multiple `.reduce()` calls into a single loop
  // to avoid redundant iterations and excessive temporary function allocations, improving performance on frequent re-renders.
  let allocated = 0;
  let spent = 0;
  let swept = 0;
...
export function getCommitmentsTotal(macroType) {
  // Optimization: Replaced `.filter().reduce()` with a single `for` loop
  // to eliminate intermediate array creation and improve garbage collection efficiency.
  let sum = 0;
  for (let i = 0; i < _state.commitments.length; i++) {
    const c = _state.commitments[i];
