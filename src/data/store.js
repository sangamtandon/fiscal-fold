...
export function getBuckets(macroType) {
  ...
  // ⚡ Optimization: Replaced chained .filter() with single-pass loop
  const buckets = [];
...
export function getMacroReserved(macroType) {
  // ⚡ Optimization: Replaced chained .filter().reduce() with single-pass loop
  let sum = 0;
...
export function getSafeToSpend() {
  ...
  // ⚡ Optimization: Replaced .reduce() with single-pass loop
  let remaining = 0;
...
export function getMacroSummary(macroType) {
  ...
  // ⚡ Optimization: Replaced multiple .reduce() calls with a single-pass loop
  let allocated = 0;
  let spent = 0;
  let swept = 0;
