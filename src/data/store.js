...
export function getBuckets(macroType) {
  ...
  const result = [];
  // Performance optimization: Single-pass for loop avoids GC overhead of intermediate arrays created by .filter()
...
export function getTransactions({ limit, bucketId } = {}) {
  const cycleId = _state.currentCycleId;
  let txns = [];
  // Performance optimization: Single-pass for loop avoids GC overhead of chained .filter() calls
...
export function getMacroReserved(macroType) {
  let sum = 0;
  // Performance optimization: Single-pass for loop avoids GC overhead of .filter().reduce()
...
export function getSafeToSpend() {
  ...
  let remaining = 0;
  // Performance optimization: Single-pass for loop avoids GC overhead of .reduce()
...
export function getMacroSummary(macroType) {
  ...
  let spent = 0;
  let swept = 0;
  // Performance optimization: Single-pass for loop avoids GC overhead of multiple .reduce() calls
