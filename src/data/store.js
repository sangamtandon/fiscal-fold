...
export function getBuckets(macroType) {
  ...
  // Performance optimization: Avoid chained array methods to reduce GC overhead during frequent re-renders
  const buckets = [];
...
export function getQuickBuckets() {
  const cycleId = _state.currentCycleId;
  // Performance optimization: Avoid array methods to reduce GC overhead during frequent re-renders
  const quickBuckets = [];
  for (let i = 0; i < _state.buckets.length; i++) {
    const b = _state.buckets[i];
...
export function getMacroSummary(macroType) {
  ...
  // Performance optimization: Avoid multiple array passes to reduce GC overhead during frequent re-renders
  let allocated = 0;
  let spent = 0;
  let swept = 0;
