...
export function getBuckets(macroType) {
  ...
  // ⚡ Bolt: Single-pass for loop avoids chained .filter() intermediate array GC overhead
  const buckets = [];
...
export function getMacroSummary(macroType) {
  ...
  // ⚡ Bolt: Single-pass for loop avoids multiple .reduce() iterations and GC overhead
  let allocated = 0;
  let spent = 0;
  let swept = 0;
